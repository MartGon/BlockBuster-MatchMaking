...
async fn main() {
    ...
    let game_sem_table = matchmaking::entity::GameSemTable::new();
    ...
    game_table.insert(sample_game.id.clone(), sample_game.clone());
    game_sem_table.insert(sample_game.id, game_sem);

    // GET /hello/warp => 200 OK with body "Hello, warp!"
    let hello = warp::path!("hello" / String).map(|name| format!("Hello, {}!", name));
    
    let routes = endpoints::filters::get_routes(player_table, game_table, player_game_table, game_sem_table).or(hello);
